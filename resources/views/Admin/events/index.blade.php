...
@php
    $Auth_User = auth()->user();
    $Is_Add = $Auth_User->HasPermission('Events', 'Is_Add');
@endphp
<div class="page-wrapper">
    <div class="page-content">
        <!--breadcrumb-->
        <div class="page-breadcrumb d-none d-sm-flex align-items-center mb-3">
            <div class="breadcrumb-title pe-3">Events
            </div>
            <div class="ps-3">
...
@push('js')
<script>
    // Example: Open offcanvas and load event details via AJAX
    const showEventOffcanvas = (eventId) => {
        showDynamicComponent(
            'eventOffcanvas', // offcanvas ID
            'eventOffcanvasContent', // content container ID
            `{{ route('events.show', ':id') }}`.replace(':id', eventId),
            'offcanvas'
        );
    };
</script>
