...
@section('title','Blog Sub Categories')
@include('Admin.Elements.datatable-css')
@section('content')
@php
    $Auth_User = auth()->user();
    $Is_Add = $Auth_User->HasPermission('Blog_Sub_Categories', 'Is_Add');
@endphp
<div class="page-wrapper">
    <div class="page-content">
        <!--breadcrumb-->
        <div class="page-breadcrumb d-none d-sm-flex align-items-center mb-3">
            <div class="breadcrumb-title pe-3">Blog Sub Categories
            </div>
            <div class="ps-3">
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb mb-0 p-0">
                        ...
                        <li class="breadcrumb-item active" aria-current="page">Blog Sub Categories List</li>
            ...
            @if($Is_Add)
                <a href="{{ route('blog_sub_category.create',$Random_Id) }}" class="btn btn-primary float-end">Add</a>
            @endif
        </div>
        <!--end breadcrumb-->
        <div class="main-body">
    ...
    <script type="text/javascript">
        $(document).ready(function()
        {
            ...
            $('#datatable').DataTable({
                info : false,
                ajax : {
                    url : "{{ route('blog_sub_category.axios_record') }}",
                    method : 'POST',
                    data : function(d)
                    {
                        d.Random_Id = "{{ $Random_Id ?? 0 }}";
                    }
                },
                order : [[ 0, "DESC" ]],
                bPaginate : true,
            ...
            $(document).on('click', '.status-button', function(e)
            {
                ...
                const axios_request = sendAxiosRequest({
                    url : "{{ route('blog_sub_category.status') }}",
                    data : formData,
                });
        ...
        function Delete_Entity(id)
        {
           $.confirm({
                ...
                buttons : {
                    ...
                    {
                        const axios_request = sendAxiosRequest({
                            url : "{{ route('blog_sub_category.delete') }}",
                            data : { id : id },
                        });
