                ...
                <div class="col-xl-12 mx-auto">
                    <div class="card">
                        <div class="card-header px-4 py-3">
                            <h5 class="mb-0">
                                <i class="bx bx-user"></i> Edit Product
                                    ...
                                    <div class="col-lg-8">
                                        <div class="border border-3 p-4 rounded">
                                            <div class="mb-3">
                                                ...
                                                <input type="text" name="Product_Name" value="{{ old('Product_Name', $entity->Product_Name) }}" class="form-control" id="Product_Name" placeholder="Enter product title" required />
                                            </div>
                                            <div class="mb-3">
                                                    ...
                                                    <div class="col-md-6">
                                                        ...
                                                        <div class="image-container">
                                                            <label for="fileInput" class="image-preview">
                                                                @if(!empty($entity->File_Name) && (File::exists(public_path('/Uploads/Products/'.$entity->File_Name))))
                                                                    @php
                                                                        $is_primary_required = true;
                                                                    @endphp
                                                                    <img id="previewImage" src="{{ asset('Uploads/Products/'.$entity->File_Name) }}" alt="Select an image">
                                                                @else
                                            ...
                                            <div class="mb-3 ">
                                                <div class="row" id="imageUploadContainer">
                                                    @if($entity->ProductFile->isNotEmpty())
                                                        @php
                                                            $imageCount = 1;
                                                        @endphp
                                                            ...
                                                            <div class="col-md-6 image-template" >
                                                                ...
                                                                <div class="image-container">
                                                                    <label for="fileInput{{ $imageCount }}" class="image-preview">
                                                                        @if(!empty($productFile->File_Name) && (File::exists(public_path('/Uploads/Products/'.$productFile->File_Name))))
                                                                            <img id="previewImage{{ $imageCount }}" src="{{ asset('Uploads/Products/'.$productFile->File_Name) }}" alt="Select an image">
                                                                        @else
                                                                            <img id="previewImage{{ $imageCount }}" src="{{ asset('Uploads/image_placeholder.jpg') }}" alt="Select an image">
                                                                        @endif
                                                                    </label>
                                                                    <input type="file" name="Secondary_File[{{ $productFile->id }}]" id="fileInput{{ $imageCount }}" accept="image/*" style="display: none;">
                                        ...
                                        <div class="border border-3 p-4 rounded">
                                            <div class="row g-3">
                                                <div class="col-md-12">
                                                    ...
                                                    <input type="text" name="Price" value="{{ old('Price', $entity->Price) }}" class="form-control" id="Price" placeholder="00.00" />
                                                </div>
                                                <div class="col-md-12">
                                                    <label for="Old_Price" class="form-label">Old Price</label>
                                                    <input type="text" name="Old_Price" value="{{ old('Old_Price', $entity->Old_Price) }}" class="form-control" id="Old_Price" placeholder="00.00" required/>
                                                </div>
                                                <div class="col-md-12">
                                                    <label for="Quntity" class="form-label">Quntity</label>
                                                    <input type="number" name="Quntity" value="{{ old('Quntity', $entity->Quntity) }}" class="form-control" id="Quntity" placeholder="1" />
                                                </div>
                                                <div class="col-md-12">
                                                    <label for="Sku" class="form-label">Sku</label>
                                                    <input type="text" name="Sku" value="{{ old('Sku', $entity->Sku) }}" class="form-control" id="Sku" placeholder="PR12345678" />
                                                </div>
                                                <div class="col-md-12">
                                                    ...
                                                    <select class="form-select" id="Product_Category_Id" name="Product_Category_Id">
                                                        ...
                                                        @if(!empty($product_categories))
                                                            @foreach($product_categories as $product_key => $product_cat)
                                                                <option value="{{$product_key}}" {{ ($product_key == $entity->Product_Category_Id) ? 'selected' : '' }}>{{ $product_cat }}</option>
                                                            @endforeach
                                                        @endif
                                                    </select>
                                                </div>
                                                ...
                                                <div class="col-12">
                                                    ...
                                                    <textarea class="form-control" id="Tags" rows="3" name="Tags" placeholder="Enter Product Tags">{{ old('Tags', $entity->Tags) }}</textarea>
                                                </div>
                                                <div class="col-12">
                ...
                if (response.status == 200)
                {
                    if (response.data.status === "success" && response.data.entities)
                    {
                        ...
                        Object.entries(response.data.entities).forEach(([key, value]) =>
                        {
                            ...
                            option.value = key;
                            option.selected = (key == {{ $entity->Product_Sub_Category_Id ?? 0 }}) ? true : false;
