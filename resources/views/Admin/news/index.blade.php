...
@php
    $Auth_User = auth()->user();
    $Is_Add = $Auth_User->HasPermission('News', 'Is_Add');
@endphp
<div class="page-wrapper">
    <div class="page-content">
        <!--breadcrumb-->
        <div class="page-breadcrumb d-none d-sm-flex align-items-center mb-3">
            <div class="breadcrumb-title pe-3">News
            </div>
            <div class="ps-3">
...
@push('js')
    {!! $dataTable->scripts() !!}
<script>
    // Example: Open offcanvas and load news details via AJAX
    const showNewsOffcanvas = (newsId) => {
        showDynamicComponent(
            'newsOffcanvas', // offcanvas ID
            'newsOffcanvasContent', // content container ID
            `{{ route('news.show', ':id') }}`.replace(':id', newsId),
            'offcanvas'
        );
    };
</script>
