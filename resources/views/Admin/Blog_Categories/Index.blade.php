...
@php
    $Auth_User = auth()->user();
    $Is_Add = $Auth_User->HasPermission('Blog_Categories', 'Is_Add');
@endphp
<div class="page-wrapper">
    <div class="page-content">
        <!--breadcrumb-->
        <div class="page-breadcrumb d-none d-sm-flex align-items-center mb-3">
            <div class="breadcrumb-title pe-3">Blog Categories
            </div>
            <div class="ps-3">
            ...
            @if($Is_Add)
                <a href="{{ route('blog_category.create') }}" class="btn btn-primary float-end">Add</a>
            @endif
        </div>
        <!--end breadcrumb-->
        <div class="main-body">
    ...
    <script type="text/javascript">
        $(document).ready(function()
        {
            ...
            $('#datatable').DataTable({
                info : false,
                ajax : {
                    url : "{{ route('blog_category.axios_record') }}",
                    method : 'POST',
                    data : function(d)
                    {
                        d.Random_Id = 0;
            ...
            $(document).on('click', '.status-button', function(e)
            {
                ...
                const axios_request = sendAxiosRequest({
                    url : "{{ route('blog_category.status') }}",
                    data : formData,
                });
        ...
        function Delete_Entity(id)
        {
           $.confirm({
                ...
                buttons : {
                    ...
                    {
                        const axios_request = sendAxiosRequest({
                            url : "{{ route('blog_category.delete') }}",
                            data : { id : id },
                        });
