...
class ProductCategoriesController extends Controller
{
    ...
    public function Edit(Request $request, $Random_Id)
    {
        try
        {
            ...
            return view('Admin.Product_Categories.Edit',compact('entity'));
        }
        ...
        foreach($records as $record)
        {
            ...
            $actions = '<div class="col">';
                $actions .= '<div class="btn-group" role="group" aria-label="Basic example">';
                    if($Is_Edit)
                    {
                        $actions .= '<a href="'.route('product_category.edit',$Random_Id).'" class="btn btn-outline-secondary"><i class="bx bx-edit"></i>Edit</a>';
                    }

                    if($Is_Read)
                    {
                        $actions .= '<a href="'.route('product_sub_category.index',$Random_Id).'" class="btn btn-outline-secondary"><i class="bx bx-edit"></i>Product Sub Category</a>';
                    }
    ...
    public function Store(Request $request)
    {
        try
        {
            ...
            return response()->json([
                ...
                'redirect' => redirect()->intended(route('product_category.index'))->getTargetUrl(),
            ], 200);
        }
        catch (\Illuminate\Database\Eloquent\ModelNotFoundException $e)
        {
    ...
    public function Axios_Product_Sub_Category(Request $request)
    {
        try
        {
            ...
            $entities = ProductCategory::where('Parent_Id',$Product_Cat_Id)->pluck('Title','id')->toArray();
            return response()->json([
                'status' => 'success',
